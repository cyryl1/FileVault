...
class FileService:
    ...
    def upload_file(self, user_id: str, file: UploadFile, parent_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not validate_file_type(file.filename):
            raise ValueError(f"File type not allowed: {Path(file.filename).suffix}")
        if parent_id:
            parent = self.file_repo.find_by_user_and_id(parent_id, user_id)
            if not parent:
                raise ValueError(f"Parent folder not found: {parent_id}")
            if parent["type"] != FileType.FOLDER.value:
                raise ValueError("Parent must be a folder")
        existing = self.file_repo.find_by_name_and_parent(file.filename, parent_id, user_id)
        if existing:
            raise ValueError(f"File '{file.filename}' already exists in the location")
        file_id = generate_file_id()
        file_type = get_file_type(file.filename)
        dest_path = self.uploads_dir / f"{file_id}_{file.filename}"
    ...
    def get_file_info(self, file_id: str, user_id: str) -> Dict[str, Any]:
        ...
        file_info = self.file_repo.find_by_id(file_id)
        if not file_info:
            raise FileNotFoundError(f"File not found: {file_id}")

        if file_info["visibility"] == Visibility.PRIVATE.value:
            if not user_id or file_info["user_id"] != user_id:
    ...
    def get_file_path(self, file_id: str, user_id: Optional[str] = None) -> str:
        ...
        if not file_info["path"]:
            raise FileNotFoundError("File data not available")
        
        return file_info["path"]

    def get_thumbnail_path(self, file_id: str, user_id: Optional[str] = None) -> str:
